...
//root app
app.get("/", (req, res) => {
  res.send("Assignment 12 by Programming Hero");
});

// laptop mania app

client.connect((err) => {
  const productCollection = client.db("dbLaptop").collection("dbLaptop");
  ...
  console.log(
    `MongoDb connected for assignment 12 given by programming hero`.magenta
  );
  ...
  app.post("/addProduct", (req, res) => {
    ...
    adminCollection.insertOne(newProduct).then((result) => {
  ...
  app.delete("/userDelete/:id", (req, res) => {
    ...
    userCollection
      .deleteOne({ key: ObjectId(req.params.id) })
      .then((result) => {
        console.log(result);
        res.send(result.deletedCount > 0);
      });
